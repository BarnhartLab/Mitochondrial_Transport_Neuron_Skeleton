...
%% directory name containing multiple .net files
% each .net file needs to contain the following saved variables:
% NT = a network object, already arranged to be a directed tree, rootnode set
% umperpx = conversion factor, micrometers per pixel
dirname = '/Users/jordankalai/Documents/Barnhart_Lab_Code/Architecture_analysis/architecture_pythonCodes/pythonDemoScripts/sym_sim_skel/';
%% load in all .mat files within the directory
% and save data
files = dir([dirname '*.net'])
...
%% clean up the loaded networks
for fc = 1:length(allNetworks)
    NT = allNetworks(fc);

    %find a node with outgoing edges only and set it to be the root
    rootnote = setRootNode(NT);

    %get rid of all degree 2 nodes
    NT.mergeAllEdgePaths()

    %set up directed tree
    %[isset,wasreversed] = directedTreeEdges(NT,1,false(1,NT.nedge));
    directedTreeEdges(NT, 1, false(1, NT.nedge));

    %% make all edge lengths the same
    NT.edgelens(1:end) = mean(NT.edgelens);
end
...
for nc = 1:length(allNetworks)    
    ...
    for s = 1: length(splittype)
        for a = 1: length(alpha)
            for b = 1: length(beta)
                number = [number nc];
                cell_type = [cell_type "simulated"]; %can change based on cell type
...
writetable(output_table,'/Users/jordankalai/Documents/Barnhart_Lab/Paper Figures/simulated_sym_model_predictions.csv','Delimiter',',')