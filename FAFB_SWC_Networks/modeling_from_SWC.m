...
addpath('/Users/jordankalai/Documents/Barnhart_Lab_Code/Architecture_analysis/architecture_measurementEditor/treestoolbox-master');
...
dirname = '/Users/jordankalai/Documents/Barnhart_Lab/MCFO/HS/SWC_skeletons/FAFB/';

%% load in all .mat files within the directory
% and save data
%start trees toolbox to load .swc
start_trees

files = dir([dirname '*.swc'])
...
for fc = 1:length(filenames)
    ...
    disp(fname)
    [exTree, name, path] = load_tree(fname);
    [NT,mergednodes] = convertSWC2NT(exTree); 
    
    allNetworks(fc) = NT;    
    allumperpx(fc) = 1; %2.091; %for the Cuntz dataset they all have the same umperpx
    %for FAFB the umperpx is somewhat random, keeping same as Cuntz for now
end
...
%writetable(output_table,'/Users/jordankalai/Documents/Barnhart_Lab/Paper Figures/FAFB_model_predictions.csv','Delimiter',',')
%writetable(output_table,'/Users/jordankalai/Documents/Barnhart_Lab/Paper Figures/simulated_model_predictions.csv','Delimiter',',')
%writetable(output_table,'/Users/jordankalai/Documents/Barnhart_Lab/Hemibrain/FB/dFB/data_analysis/dFB_model_predictions.csv','Delimiter',',')